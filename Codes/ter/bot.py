...
class TERBot(commands.Bot):
    def __init__(
        ...
        broadcaster_user_name: str = (
            str(self.__j["messageChannel"]["broadcasterUserName"])
            .casefold()
            .strip()
        )
    ...
    async def event_ready(self) -> None:
        ...
        if name_color != "doNotChange".casefold().strip():
            print(
                f"    Setting bot name color = {name_color} ... ",
                end="",
            )
            #
    ...
    async def __test(self, ctx: commands.Context):
        print(
            f"  Testing bot (v{str(self.__j['verNo']).casefold().strip()}) ..."
        )
    ...
    async def __kill(self, ctx: commands.Context):
        if self.__is_by_channel_broadcaster_or_myself(ctx) is True:
            ...
            return_code_str: str = (
                ...
                .strip()
                .removeprefix(f"{self.__prefix}kill")
                .strip()
            )
            if (
                len(return_code_str) <= 3
                and return_code_str.isdecimal() is True
            ):
            ...
            print(
                f"    Return code = {self.__return_code}",
                end="",
            )
    ...
    def __is_by_channel_broadcaster_or_myself(
        self,
